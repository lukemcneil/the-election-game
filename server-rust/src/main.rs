...
use types::{Answer, Game, Guess, Player, PlayerData, PromptData, Result};
...
fn chat_gpt_question(
    ...
    game.change_question(
        questions
            .lock()
            .get_gpt_question(&prompt.into_inner().prompt),
    );
    Ok(())
}
...
fn rocket(opt: Option<Opt>) -> rocket::Rocket {
    ...
    rocket
        ...
        .mount("/pictures", StaticFiles::from("pictures").rank(11))
        .mount(
            "/api/v1",
            routes![
                heartbeat,
